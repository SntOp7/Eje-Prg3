defmodule Proceso do
  ...
  def simulacion(cantidad_procesos_internos) do
    ...
    tarea = Enum.map(datos_prueba, fn valor ->
      Task.async(fn -> simulando_proceso(valor, cantidad_procesos_internos) end) end)
  ...
  def simulando_proceso({mensaje, demora}, cantidad_procesos_internos) do
    ...
    IO.puts("#{mensaje} -> (Finalizado)")
  end

  def generar_mensaje(t), do: "\nEL tiempo de ejecucion es de #{t} microsegundos."
  end
